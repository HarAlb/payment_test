...
namespace App\Services\Payment\Strategies\FirstStrategy;
...
class FirstStrategy implements PaymentStrategy
{
    public function confirm(PaymentDTO $paymentDto)
    {
        $existsID = Payment::where([
            ...
            'status_id' => PaymentStatus::where('name', PaymentENUM::PENDING)->value('id')
        ])->value('id');
        if (!$existsID) {
